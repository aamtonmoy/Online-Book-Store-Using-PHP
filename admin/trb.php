<!DOCTYPE html>
<html>
<head>
	<title>Home</title>
	<link rel="stylesheet" type="text/css" href="css/s1.css">
</head>
<body style="background-image: url('img/1.jpg'); width: 100%;">

<div id="header">
	<h2 style="padding: 30px"><font face="Cooper Std"><i>Pick-a-Book</i></font></h2>
</div>
<div id="nav">
	<ul>
		<li><b><a href="ahome.php">Home</a></b></li>
		<li><b><a href="ib.php">Insert Book</a></b></li>
		<li><b><a href="bl.php">Book List</a></b></li>
		<li><b><a href="sb.php">Sell Book</a></b></li>
		<li><b><a href="help.php">Help</a></b></li>
	</ul>
</div>
<div id="container">
	<div id="left">
		<h3 style="color: red;margin-left: 50px">Top rated Book</h3>
		<div id="a"><center><img src="img/book1.jpg" width="150" height="250"></center></div>
		<div id="b"><center><img src="img/book2.jpg" width="150" height="250"></center></div>
		<div id="c"><center><img src="img/book3.jpg" width="150" height="250"></center></div>
	</div>
    <div id="right">
    	<h1 style="color: red">Category</h1>
    	<a href="#"><b>Romantic</b></a><br><br><br><br>

    <a href="#"> <b>Biography</b></a><br><br><br><br>

     <a href="#"><b>Detective</b></a><br><br><br><br>

     <a href="#"><b>Suspense</b></a>

    </div>
</div>
<div id="footer"><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>
	<h1 style="margin-top: 300px; color: red" align="center"></h1>
</div>
</body>
</html>